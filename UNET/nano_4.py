...
def main():
    ...
    folder_name = "jetson_4" #EDIT CONTAINS JETSON NUMBER
    ...
    while True:
        ...
        LOAD_DIR = "/home/nano/srdsg/Senior_Design_Group_FH7/UNET/jetson_4"  #EDIT CONTAINS JETSON NUMBER
        ...
        current_dir = os.getcwd()
        if os.path.basename(current_dir) == "jetson_4": #EDIT CONTAINS JETSON NUMBER
            os.chdir('..')
            print(f"Exited {current_dir}, current working directory is now {os.getcwd()}")
        else:
            print(f"Error: current directory is not jetson_4") #EDIT CONTAINS JETSON NUMBER

        print("setting name of trained model file\n")
        trained_model_file_name = "nano_4_trained_model.pth" #EDIT CONTAINS JETSON NUMBER
        print("name of trained model is {}\n".format(trained_model_file_name))

        print("starting to training model on nano\n")
        model = TrainingLoop(2, 1, .0001, trained_model_file_name,jetson_num=4) #EDIT CONTAINS JETSON NUMBER
        print("training of model complete\n")

        print("zipping the trained model on nano\n")
        with zipfile.ZipFile("trained_model_4.zip", mode = 'w') as archive: #EDIT CONTAINS JETSON NUMBER
        ...
        file_size = os.path.getsize(r"trained_model_4.zip") #EDIT CONTAINS JETSON NUMBER
        ...
        os.remove("{}".format(trained_model_file_name))
        os.remove("{}".format("trained_model_4.zip")) #EDIT CONTAINS JETSON NUMBER
