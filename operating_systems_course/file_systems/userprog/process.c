...
process_execute (const char *file_name) 
{
  ...
  if (child_t->load_bit == -1)
    {
      list_remove (&get_thread_by_tid (thread_current (), tid) -> child_elem);
      tid = TID_ERROR;
    }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  char *str_copy = palloc_get_page (001 | 002);
  if (str_copy)
...
setup_stack (void **esp, char *file_name) 
{
  ...
  char *token;
  /* Tokenize the arguments and save them in argv. */
  ...
  while (num_args >= 0) 
  {
    ...
    if (space_on_stack (myESP, str_len))
      {
        /* Decrement stack pointer bit by bit w/ error checking for space. */
        myESP -= str_len;
        align += str_len;

        /* Pushing arg onto stack. */
  ...
  /* Check alignment. */
  int offset = POINTER_SIZE - (align % POINTER_SIZE);
  if (space_on_stack (myESP, offset + sizeof (char*)))
    {
      ...
      /* Subtract four for null pointer sentinel. */
      myESP -= POINTER_SIZE;
    }
  else
    {
  ...
  while (counter < adr_len) 
    {
      /* If space not available, free and return. */
  ...
  *esp = myESP;
  ...
  /* Free argv which was initially palloced. */
  palloc_free_page (argv);
  return true;
}
...
/* Retrieves the thread from t's children list utilizing tid. */
