...
{
    hash_init (table, page_hash, page_order, NULL);
}
...
bool
page_order (const struct hash_elem *a_, const struct hash_elem *b_,
           void *aux UNUSED)
{
...
struct page *
lookup (struct hash *table, const void *address)
{
  struct page p;
  struct hash_elem *e;
...
/* Called in install_page() to insert into the supp page table. */
bool
insert_spt (struct page *p)
{
