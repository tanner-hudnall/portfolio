...
def TrainingLoop( batch_size, epochs, lr):
    multipleDatasets = [0, 1, 2, 3, 4, 5, 6]
    model = Model.cuda()
    LOAD_DIR = "."
    ...
    #depth_dataset, test_dataset = torch.utils.data.random_split(depth_dataset, [5000,  len(depth_dataset) - 5000])
    multipleDatasets[0], test_dataset = torch.utils.data.random_split(depth_dataset, [5000, len(depth_dataset) - 5000])
    for i in range(6):
        multipleDatasets[i+1], test_dataset = torch.utils.data.random_split(test_dataset, [5000, len(test_dataset) - 5000])



    train_loader = DataLoader(random.choice(multipleDatasets), batch_size, shuffle=True)
