...
pretrained_dict = torch.load(rf"{nano_dir}/mobilenetv3-small-55df8e1f.pth")
...
def TrainingLoop( batch_size, epochs, lr, trained_model_file_name,jetson_num, verbosity):
    model = Model.cuda()
    ...
    model.load_state_dict(torch.load('UNET_MBIMAGENET.pth'))
    ...
    traincsv = pd.read_csv(rf"{nano_dir}/nyu_data/data/nyu2_train.csv") # TODO fix path when fixing directory stuff
    traincsv = traincsv.values.tolist()
    # traincsv = shuffle(traincsv, random_state=2)

    depth_dataset = DepthDataset(traincsv=traincsv, root_dir=rf"{nano_dir}/nyu_data/",
