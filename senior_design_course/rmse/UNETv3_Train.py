...
model = Model.cuda()
LOAD_DIR = "."
model.load_state_dict(torch.load('{}/UNET200.pth'.format(LOAD_DIR)))
# print('Model Loaded.')
...
traincsv = shuffle(traincsv, random_state=6)



def write_csv(data):
    with open('LossInfo.csv', 'a') as outfile:
...
depth_dataset, test_dataset = torch.utils.data.random_split(depth_dataset, [45000, len(depth_dataset) - 45000])
train_loader = DataLoader(depth_dataset, batch_size, shuffle=True)
eval_loader = DataLoader(test_dataset, 1 , shuffle=True)

print(train_loader.__len__())
print(eval_loader.__len__())
...
for epoch in range(epochs):
    ...
    torch.save(model.state_dict(), '{}/UNET200.pth'.format(LOAD_DIR))
   ...
   # Switch to train mode
    ...
    count = 0
    with torch.no_grad():
        for i, sample_batched in enumerate(eval_loader):
            image = torch.autograd.Variable(sample_batched['image'].cuda())
            depth = torch.autograd.Variable(sample_batched['depth'].cuda(non_blocking=True))
            depth_n = DepthNorm(depth)
            pred = Model(image)
            ...
            log = torch.log(depth_n) - torch.log(pred)
            square = log**2
            mean = torch.nanmean(square)
            eval_loss = torch.sqrt(mean)
            val.update(eval_loss.data.item(), image.size(0))
            if i % 5 == 0:
                print("[epoch %2d] RMSE_log: %.4f" % (epoch, val.avg))

    print("[epoch %2d] RMSE_log: %.4f" % (epoch, val.avg))

    write_csv([epoch, losses.avg, val.avg])
